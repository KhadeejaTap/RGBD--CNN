...
class RGBDDataset(Dataset):
	...
	def __getitem__(self, index):
		...
		mask = torch.from_numpy(np.load(self.mask_files[index])).unsqueeze(0)
...
depth_feats = depth_encoder(depth)
...
# fix nan in single depth sample same way dataset does in bulk
depth_s = torch.nan_to_num(depth_s, nan=0.0)
...
for i in range(n_overfit_iters):
	...
	depth_feats = depth_encoder(depth_s)
	fused = fusion(rgb_feats, depth_feats)
...
plt.imsave(f"{frame_id}_pred.png", pred_np, cmap='viridis')
plt.imsave(f"{frame_id}_error.png", error_np, cmap='hot')

print(f"saved {frame_id}_pred.png and {frame_id}_error.png")
