...
class RGBDDataset(Dataset):
	...
	def __getitem__(self, index):
		...
		# pretrained resnet18 expects inputs normalized to ImageNet's mean/std,
		# not raw 0-1 range - without this, pretrained weights see an input
...
class SparseBasicBlock(nn.Module):
	# resnet18's BasicBlock (conv-bn-relu-conv-bn + residual), but both convs are
	# sparse-invariant so validity keeps propagating instead of being dropped
	# after just one layer.
	def __init__(self, channels):
		super().__init__()
		self.sconv1 = SparseConv2d(channels, channels, kernel_size=3, stride=1, padding=1)
		self.bn1 = nn.BatchNorm2d(channels)
		self.relu = nn.ReLU(inplace=True)
		self.sconv2 = SparseConv2d(channels, channels, kernel_size=3, stride=1, padding=1)
		self.bn2 = nn.BatchNorm2d(channels)

	def forward(self, x, mask):
		identity = x
		out, mask = self.sconv1(x, mask)
		out = self.bn1(out)
		out = self.relu(out)
		out, mask = self.sconv2(out, mask)
		out = self.bn2(out)
		out = out + identity  # same shape/stride as resnet18 layer1, safe residual add
		out = self.relu(out)
		return out, mask


class SparseLayer1(nn.Module):
	# replaces resnet.layer1 (2 BasicBlocks, 64->64, no downsample) with two
	# mask-propagating blocks. If a pretrained resnet_layer1 is passed in,
	# warm-starts conv/bn weights from it instead of random init.
	def __init__(self, resnet_layer1=None):
		super().__init__()
		self.block1 = SparseBasicBlock(64)
		self.block2 = SparseBasicBlock(64)
		if resnet_layer1 is not None:
			self.block1.sconv1.conv.weight.data = resnet_layer1[0].conv1.weight.data.clone()
			self.block1.bn1.load_state_dict(resnet_layer1[0].bn1.state_dict())
			self.block1.sconv2.conv.weight.data = resnet_layer1[0].conv2.weight.data.clone()
			self.block1.bn2.load_state_dict(resnet_layer1[0].bn2.state_dict())
			self.block2.sconv1.conv.weight.data = resnet_layer1[1].conv1.weight.data.clone()
			self.block2.bn1.load_state_dict(resnet_layer1[1].bn1.state_dict())
			self.block2.sconv2.conv.weight.data = resnet_layer1[1].conv2.weight.data.clone()
			self.block2.bn2.load_state_dict(resnet_layer1[1].bn2.state_dict())
...
class DepthEncoder(nn.Module):
    def __init__(self):
        ...
        # initialize sparse conv from pretrained RGB conv
        self.sparse_conv1.conv.weight.data.copy_(
        ...
        self.l1 = SparseLayer1(resnet.layer1)
        self.l2 = resnet.layer2
        self.l3 = resnet.layer3
        self.l4 = resnet.layer4
...
def structure_distillation_loss(pred, mde, window_size=11):
	C1 = 0.01 ** 2
	C2 = 0.03 ** 2
	mde_min = mde.amin(dim=(2, 3), keepdim=True)
	mde_max = mde.amax(dim=(2, 3), keepdim=True)
	mde = (mde - mde_min) / (mde_max - mde_min + 1e-8)
	mde = mde * 2 - 1
	mde = -mde
...
def normal_consistency_loss(pred, gt, grad_scale=10.0):
	def compute_normals(depth):
		...
		dx = F.pad(dx, (0, 1, 0, 0))
		dy = F.pad(dy, (0, 0, 0, 1))
		# depth here is compressed into roughly [-1,1] range (tanh output / normalized gt),
		# so raw dx/dy are tiny compared to the hardcoded z=1 below, which makes every
		# normal collapse toward straight-up regardless of real tilt. Scale dx/dy up so
		# they're proportionally meaningful again relative to z.
...
def pad_to_multiple(x, multiple=32):
	...
	# pad on bottom/right only, keeps top-left aligned with original
	return F.pad(x, (0, pad_w, 0, pad_h)), (h, w)

rgb_padded, _ = pad_to_multiple(rgb)
depth, _ = pad_to_multiple(depth)
mask, _ = pad_to_multiple(mask)
...
rgb_feats = rgb_encoder(rgb_padded)
...
orig_h, orig_w = rgb.shape[-2:]  # unpadded rgb's real height/width
...
l_struct = structure_distillation_loss(out, mde)
...
print("testing: pad+crop, kinda degraded not rly")
